        ...
        <div class="col-md-12 mb-3">
          <div class="card">
            <div class="card-body">
              ...
              <p class="card-text">{{ \Illuminate\Support\Str::limit($article->content, 180) }}</p>
              <small class="text-muted">Published: {{ $article->published_at ?? $article->created_at->toDateString() }} • Writer: {{ $article->writer->name }}</small>
              <div><a href="{{ route('category.show', \Illuminate\Support\Str::slug($article->category)) }}" class="btn btn-sm btn-primary mt-2">Read more...</a></div>
            </div>
          </div>
        </div>
      @endforeach
